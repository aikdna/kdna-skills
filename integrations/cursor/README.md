...
Load KDNA domains in Cursor to give your agent domain-specific judgment before it acts.
...
`kdna setup` auto-detects Cursor and installs the `kdna-loader` skill.

## Verify Installation

```bash
kdna doctor --agents
# → Cursor: detected, kdna-loader installed
```

Check the skill file exists:

```bash
ls ~/.cursor/skills/kdna-loader/SKILL.md
```
